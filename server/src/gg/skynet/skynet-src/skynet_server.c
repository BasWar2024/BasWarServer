...
int
skynet_handle_exit(struct skynet_context * context, uint32_t handle) {
	if (context == NULL) {
		return skynet_handle_retire(handle);
	}
	if (handle == 0) {
	...
	// if (context->onexit != NULL) {
	// 	context->onexit(context,context->cb_ud);
	// }
	return skynet_handle_retire(handle);
}
...
cmd_kill(struct skynet_context * context, const char * param) {
	...
	if (handle) {
		skynet_handle_exit(context, handle);
	}
	return NULL;
}
...
void
skynet_onexit(struct skynet_context * context, skynet_cb_onexit onexit) {
