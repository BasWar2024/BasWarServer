...
function trace.traceline(level)
    ...
    local function hook(level)
        return function (mode,line)
            ...
            elseif mode == "line" then
                if call <= level then
                    local indent = string.rep("  ",call)
                    local frame = debug.getinfo(2)
                    local log = string.format("%s%s:%s",indent,frame.short_src,line)
                    trace.print(log)
                end
            else
                -- tail call
...
function trace.tracecall(level)
    ...
    local function hook(level)
        return function (mode)
            ...
            if deepth and call <= level then
                local indent = string.rep("  ",deepth-1)
                local frame = debug.getinfo(3)
                local my_frame = debug.getinfo(2)
                local funcname = my_frame.name or string.format("(%s)",my_frame.what)
                local log = string.format("%s%s:%s %s %s",indent,frame.short_src,frame.currentline,funcname,mode)
                trace.print(log)
            end
        end
    end
    debug.sethook(up(2,hook(level or 0)),"cr")
end
