---""(NFT)""
--@module api.analyzer.getChainWithdrawNftList
--@author sw
--@release 2022/3/12 10:30:00
--@usage
--api:      /api/analyzer/getChainWithdrawNftList
--protocol: http/https
--method:   post
--params:
...
--      token_id          [required] type=string help=token_id
--      item_id          [required] type=string help=item_id
--      item_cfgid          [required] type=string help=item_cfgid
--      to_address          [required] type=string help=""
--      state              [required] type=string help="" --0"",1"",2"",-1""
...
--  }
--example:
--  curl -v 'http://127.0.0.1:4241/api/analyzer/getChainWithdrawNftList' -d '{}'
...
function handler.exec(linkobj,header,args)
    ...
    condition.token_id = nil
    if request.token_id ~= NO and tonumber(request.token_id) then
        local token_id = math.floor(tonumber(request.token_id))
        condition.token_ids = token_id
    end

    condition.item_id = nil
    if request.item_id ~= NO and tonumber(request.item_id) then
        local item_id = math.floor(tonumber(request.item_id))
        condition.item_ids = item_id
    end

    condition.item_cfgid = nil
    if request.item_cfgid ~= NO and tonumber(request.item_cfgid) then
        local item_cfgid = math.floor(tonumber(request.item_cfgid))
        condition.item_cfgids = item_cfgid
    end
    ...
    local result = analyzermgr.getChainWithdrawNftList(pageNo, pageSize, condition)
