---""(NFT)""
--@module api.analyzer.getChainRechargeNftList
--@author sw
--@release 2022/3/12 10:30:00
--@usage
--api:      /api/analyzer/getChainRechargeNftList
--protocol: http/https
--method:   post
--params:
...
--      from_address          [required] type=string help=""
...
--  }
--example:
--  curl -v 'http://127.0.0.1:4241/api/analyzer/getChainRechargeNftList' -d '{}'
...
function handler.exec(linkobj,header,args)
    ...
    if request.order_num ~= NO then
        condition.order_num = request.order_num
    end
    ...
    condition.from_address = nil
    if request.from_address ~= NO then
    ...
    local result = analyzermgr.getChainRechargeNftList(pageNo, pageSize, condition)
