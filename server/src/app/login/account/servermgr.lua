...
function servermgr.saveserver(appid,server)
    ...
    server.appid = appid
    gg.mongoProxy[db_name]:update({appid=appid,id=id},server,true,false)
end

function servermgr.loadserver(appid,serverid)
    local app = assert(util.get_app(appid))
    local db_name = app.current_serverlist or "server"
    local doc = gg.mongoProxy[db_name]:findOne({appid=appid,id=serverid})
    if doc == nil then
        return nil
    else
        doc._id = nil
...
function servermgr.delserver(appid,serverid)
    local app = assert(util.get_app(appid))
    local db_name = app.current_serverlist or "server"
    local ok = gg.mongoProxy[db_name]:safe_delete({appid=appid,id=serverid},true)
    if not ok then
...
function servermgr.getserverlist(appid)
    ...
    local db_name = app.current_serverlist or "server"
    local serverlist = {}
    local docs = gg.mongoProxy[db_name]:find({appid=appid})
    local now = os.time()
