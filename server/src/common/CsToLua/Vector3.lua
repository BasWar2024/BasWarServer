...
function Vector3.CloneNew(clone)
    if clone then
        v = {x = clone.x, y = clone.y, z = clone.z}
    else
        v = {x = 0, y = 0, z = 0}
    end
    setmetatable(v, Vector3)
...
function Vector3.Angle(from, to)
    if from == Vector3.zero or to == Vector3.zero then
        return 0
    end
...
function Vector3.RotateAroundAxis(v3,axis,angle)
    local q = Quaternion.AngleAxis(angle,axis)
    return q * v3
end
...
function Vector3.Rotate(v3,angle)
    ...
    local x2 = math.cos(radian) * v3.x - math.sin(radian) * v3.z
    local z2 = math.sin(radian) * v3.x + math.cos(radian) * v3.z
    return Vector3.New(x2,0,z2)
end